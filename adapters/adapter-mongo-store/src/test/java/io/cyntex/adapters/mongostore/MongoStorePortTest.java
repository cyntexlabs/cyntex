...
/**
 * The aggregated store port binds the three store concerns to three distinct, named collections. The
...
class MongoStorePortTest {
    ...
    @Test
    void bindsTheThreeConcernsToThreeDistinctNamedCollections() {
        assertThat(List.of(
                MongoStorePort.ARTIFACTS,
                MongoStorePort.PIPELINE_STATE,
                MongoStorePort.CONNECTIONS))
                .doesNotHaveDuplicates()
                .containsExactly("artifacts", "pipeline_state", "connections");
    }
}

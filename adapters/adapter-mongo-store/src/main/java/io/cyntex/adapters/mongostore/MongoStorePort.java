...
/**
 * The MongoDB implementation of the persistence port: it aggregates the three sub-stores — the
 * artifact truth layer, the epoch-fencing pipeline state store, and the connection catalog — each
 * bound to its own collection on the verified connection's database. This is the store bridge the
...
public final class MongoStorePort implements StorePort {
    ...
    /**
     * Binds the three sub-stores to their own collections on the verified connection's database. The
     * connection must have been verified first (its client opened); the three sub-stores share that
